...
// 
CommProtocol::CommProtocol(CommHandler * handler, SerPort * serPort) //: Process("CommProtocol")
{
...
// 
void CommProtocol::WriteBlock(UInt8 frameType, void * dataPtr, UInt32 len)
{
	...
	if (len > kBufferSize - kPacketMinSize)
		return;
		...
		case 2:
		{
			...
			if (inLen >= mExpectedLen - 2)
			{
				...
				if (crc == crcReceived)
				{
					...
					{
						UInt16 diff;
						diff = (UInt16)mExpectedLen - kPacketMinSize;

						mHandler->HandleComm(mInData[2], &(mInData[3]), diff);
					}
